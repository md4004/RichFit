...
function UpdateLockHandler() {
  ...
  useEffect(() => {
    const checkUpdate = async () => {
      // Tactical lock for Android users, but check for all logged-in sessions
      if (!user) return;
      ...
      try {
        // Bypass caching using timestamp
        const response = await fetch(`${VERSION_JSON_URL}?t=${Date.now()}`);
        if (!response.ok) return;
        const data = await response.json();
        
        if (data.version && data.version !== CURRENT_VERSION) {
          console.log("CRITICAL UPDATE DETECTED:", data.version);
          setUpdateDetails(data);
          setShowUpdateLock(true);
          document.body.style.overflow = 'hidden';
        }
      } catch (error) {
        console.error("Update protocol sync failed:", error);
      }
    };
          ...
          <div className="max-w-md w-full border-2 border-primary shadow-[0_0_50px_rgba(255,0,0,0.3)] bg-zinc-950 p-8 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <a 
                ...
                className="w-full bg-primary text-black font-headline font-black text-lg py-5 uppercase tracking-tighter hover:bg-white transition-all shadow-[0_0_30px_rgba(255,0,0,0.4)] animate-bounce flex items-center justify-center gap-3"
              >
